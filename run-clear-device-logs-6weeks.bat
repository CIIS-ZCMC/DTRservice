...
:: ============================================================================
:: ZCMC DTRService - Biometric Device Attendance Logs Clearance (6 Weeks)
:: ============================================================================
:: Purpose: Safely clears attendance logs on physical biometric devices that
::          have not been cleared in 6 weeks (42 days).
...
echo ============================================================================
echo  ZCMC DTRService - Clearing Device Attendance Logs (Older than 6 Weeks)
echo  Started: %date% %time%
echo ============================================================================

echo [%date% %time%] [TASK] Starting 6-week biometric device log clearance... >> "%~dp0storage\logs\device_clear.log"

:: Run artisan command and append output to log file while displaying output
"%PHP_BIN%" artisan devices:clear-logs --all --catch-up --older-than=42 --force --method=both %* >> "%~dp0storage\logs\device_clear.log" 2>&1
...
if %CMD_STATUS% equ 0 (
    ...
    echo [%date% %time%] [SUCCESS] 6-week biometric device log clearance completed successfully. >> "%~dp0storage\logs\device_clear.log"
) else (
    ...
    echo [%date% %time%] [ERROR] 6-week biometric device log clearance exited with error code %CMD_STATUS%. >> "%~dp0storage\logs\device_clear.log"
)
